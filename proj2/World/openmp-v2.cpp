        ...
        {
            {
                ...
                {
                    int threadID = omp_get_thread_num();  // Each thread has a seperate list
                    bin_type temp = temp_vector[threadID];
                    temp.clear();
                    #pragma omp for
                    for (int i = 0; i < binNo; i++) {
                        for(int j = 0; j < binNo; j++) {
                    ...
                    #pragma omp master 
                        ...
                        temp.clear(); //free the items in the temp list
                        }
            ...
            #pragma omp barrier // Wait until all threads complete, especially the master thread that costs the longest.
        }
    }
    simulation_time = read_timer( ) - simulation_time;
    
    printf("-------------------Parelleled by %d threads------------------\n", numthreads);
