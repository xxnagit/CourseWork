...
void grid_init(grid_t & grid, int size){
    ...
    if (grid.grid == NULL)
    {
        fprintf(stderr, "Error: There is an error while allocating memory for the grid!\n");
        exit(1);
    }

    memset(grid.grid, 0, sizeof(linkedlist*) * size * size); //make sure the grid is reset to 0
}
...
{
    int gridCoord = grid_coord_flat(grid.size, p->x, p->y); //(*p).x, (*p).y
